...
/// a CUT/COPYCLIP operation when all member entities in the
/// group are included in the operation. 
/// 
...
namespace AcMgdLib.GroupExtensions
{
   public class WblockGroupHandler : WblockCloneHandler
   {
      ...
      {
         SupportedCommands.Add("WBLOCK", "COPYCLIP", 
      ...
      protected override void OnDeepCloneEnded(IdMapping map, bool aborted)
      {
         ...
         /// Example code showing how to determine if objects are 
         /// being pasted into same drawing they were copied from:
         using(var tr = new OpenCloseTransaction())
         {
            var targetDb = map.DestinationDatabase;
            var bt = tr.GetObject<BlockTable>(targetDb.BlockTableId, OpenMode.ForWrite);
            bt.SetXData("APPID_WBLOCKSOURCE", tr, 
               new TypedValue(
                  (short)DxfCode.ExtendedDataAsciiString, 
                  map.OriginalDatabase.FingerprintGuid));
            tr.Commit();
         }
         ...
         try
         {
            using(var trans = new OpenCloseTransaction())
            {
               ...
               foreach(ObjectId id in clonableGroupIds)
               {
                  ...
                  {
                     CloneGroup(groups, source, cloneIds, trans);
                     ++cloned;
                  }
               }
               trans.Commit();
      ...
      Group CloneGroup(DBDictionary owner, Group source, ObjectIdCollection cloneIds, Transaction tr)
      {
